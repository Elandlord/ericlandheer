        ...
        <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
            <div class="flex flex-wrap items-end justify-between gap-6" v-reveal>
                <div class="max-w-xl">
                    ...
                    <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-white sm:text-5xl">Recent repositories.</h2>
                    <p class="mt-4 text-slate-300">Fresh from <a href="https://github.com/Elandlord" target="_blank" rel="noreferrer" class="text-accent hover:underline">@Elandlord</a> — updated automatically.</p>
                </div>
                <a
            ...
            <div v-if="pending" class="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                <div v-for="n in 6" :key="n" class="h-44 animate-pulse rounded-2xl border border-white/5 bg-white/[0.02]" />
            </div>
            ...
            <div v-else-if="!repos?.length" class="mt-12 rounded-2xl border border-white/10 bg-white/[0.02] p-8 text-center text-sm text-slate-400">
                ...
                <a href="https://github.com/Elandlord" target="_blank" rel="noreferrer" class="text-accent hover:underline">Visit GitHub</a> instead.
            </div>
            ...
            <div v-else class="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                <a
                    ...
                    rel="noreferrer"
                    v-reveal="i * 50"
                    class="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.04] to-transparent p-6 transition hover:border-accent/40"
                >
                    ...
                    <div class="relative">
                        <div class="flex items-center justify-between gap-3">
                            <h3 class="font-display text-lg font-semibold text-white truncate">{{ repo.name }}</h3>
                        ...
                        <p v-if="repo.description" class="mt-3 line-clamp-3 text-sm leading-relaxed text-slate-400">{{ repo.description }}</p>
                    </div>
                    <div class="relative mt-5 flex flex-wrap items-center gap-3 text-xs text-slate-400">
